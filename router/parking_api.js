...
Router.post('/addparking', async (req, res) => {
    ...
    try {
        ...
        let parking_obj = {}
        if (req.body.name && req.body.title && req.body.total_slots && req.body.booked_slots && req.body.description && req.body.photos && req.body.city && req.body.country && req.body.area) {
        ...
        //
        const exists = await Parking.findOne({ name: parking_obj.name, city: parking_obj.city, area: parking_obj.area });
        if (exists) {
